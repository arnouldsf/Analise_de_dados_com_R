...
# No entanto, tambem podemos mapear uma variável continua a cor dos pontos:
ggplot(mtcars, aes(x = disp, y = mpg, colour = cyl)) + geom_point()


# Tambem podemos mapear o tamanho dos pontos a uma variável de interesse:
# A legenda é inserida no gráfico automaticamente
ggplot(mtcars, aes(x = disp, y = mpg, colour = cyl, size = wt)) + geom_point()


# Os geoms definem qual forma geométrica sera utilizada para a visualizacao dos dados no grafico. 
...
ggplot(mtcars, aes(x = mpg)) + geom_histogram() + xlab("Milhas por galao") + ylab("Frequência") +
  xlim(c(0, 40)) +
  ylim(c(0,8))
